...
It's often useful to know where the mixture components are location, so we can get their posterior means using the `component_mean_samples` property for a given number of components. Here we'll calculate the median posterior means of the components for the mixture model with 3 components:
