...
We can also visualise the mixture model for a given number of components using the `plot_mixture()` function
...
plot(igraph::E(bison_to_igraph(fit_edge)[[1]])$weight, get_edgelist((fit_edge))$median)
```
